...
?>
			...
			<div <?php post_class(); ?> id="post-<?php the_ID(); ?>">
					...
					<dl class="article-info">
						...
						<?php if( $gantry->get( 'single-meta-comments-enabled', '1' ) ) : ?>
							...
							<?php else : ?>
								...
								<dd class="comments-count"> <?php comments_number( __( '0 Comments', 'rt_gantry_wp_lang' ), __( '1 Comment', 'rt_gantry_wp_lang' ), __( '% Comments', 'rt_gantry_wp_lang' ) ); ?></dd>
				...
				<?php wp_link_pages( 'before=<div class="pagination page-pagination">' . __( 'Pages:', 'rt_gantry_wp_lang' ) . '&after=</div>' ); ?>

				<?php edit_post_link( __( 'Edit', 'rt_gantry_wp_lang' ), '<div class="edit-link">', '</div>' ); ?>
				...
				<?php if( $gantry->get( 'single-footer', '1' ) ) : ?>
					...
					<div class="post-footer">
						...
						<?php _e( 'This entry was posted', 'rt_gantry_wp_lang' ); ?>
						<?php /* This is commented, because it requires a little adjusting sometimes.
						You'll need to download this plugin, and follow the instructions:
						http://binarybonsai.com/archives/2004/08/17/time-since-plugin/ */
						/* $entry_datetime = abs(strtotime($post->post_date) - (60*120)); echo time_since($entry_datetime); echo ' ago'; */ ?>
						<?php _e( 'on', 'rt_gantry_wp_lang' ); ?> <?php the_time('l, F jS, Y') ?> <?php _e( 'at', 'rt_gantry_wp_lang' ); ?> <?php the_time() ?>
						<?php _e( 'and is filed under', 'rt_gantry_wp_lang' ); ?> <?php the_category(', ') ?>.
						<?php _e( 'You can follow any responses to this entry through the', 'rt_gantry_wp_lang' ); ?> <?php post_comments_feed_link('RSS 2.0'); ?> <?php _e( 'feed', 'rt_gantry_wp_lang' ); ?>.

						<?php if (('open' == $post->comment_status) && ('open' == $post->ping_status)) {
						// Both Comments and Pings are open ?>
						<?php _e( 'You can', 'rt_gantry_wp_lang' ); ?> <a href="#respond"><?php _e( 'leave a response', 'rt_gantry_wp_lang' ); ?></a>, <?php _e( 'or', 'rt_gantry_wp_lang' ); ?> <a href="<?php trackback_url(); ?>" rel="trackback"><?php _e( 'trackback', 'rt_gantry_wp_lang' ); ?></a> <?php _e( 'from your own site.', 'rt_gantry_wp_lang' ); ?>

						<?php } elseif (!('open' == $post->comment_status) && ('open' == $post->ping_status)) {
						// Only Pings are Open ?>
						<?php _e( 'Responses are currently closed, but you can', 'rt_gantry_wp_lang' ); ?> <a href="<?php trackback_url(); ?> " rel="trackback"><?php _e( 'trackback', 'rt_gantry_wp_lang' ); ?></a> <?php _e( 'from your own site.', 'rt_gantry_wp_lang' ); ?>

						<?php } elseif (('open' == $post->comment_status) && !('open' == $post->ping_status)) {
						// Comments are open, Pings are not ?>
						<?php _e( 'You can skip to the end and leave a response. Pinging is currently not allowed.', 'rt_gantry_wp_lang' ); ?>

						<?php } elseif (!('open' == $post->comment_status) && !('open' == $post->ping_status)) {
						// Neither Comments, nor Pings are open ?>
						<?php _e('Both comments and pings are currently closed.', 'rt_gantry_wp_lang' ); ?>

						<?php } edit_post_link(__( 'Edit this entry', 'rt_gantry_wp_lang' ),'','.'); ?>
